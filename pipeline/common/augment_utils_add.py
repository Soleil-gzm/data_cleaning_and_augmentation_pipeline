...
if os.path.exists(SIMILARWORD_DICT_PATH):
    try:
        _similarword_aug = Similarword(
            base_file=SIMILARWORD_DICT_PATH, create_num=3, change_rate=0.2, seed=42
        )
...
if not os.path.exists(HOMOPHONE_DICT_PATH):
    ...
    try:
        _homophone_aug = Homophone(
            base_file=HOMOPHONE_DICT_PATH, create_num=3, change_rate=0.3, seed=42
        )
...
if os.path.exists(ENTITY_FILE):
    try:
        _random_entity_aug = Randomword(
            base_file=ENTITY_FILE, create_num=3, change_rate=0.2, seed=42
        )
...
def multi_step_augment(sentence: str, min_steps=1, max_steps=3, weights=None) -> str:
    ...
    if weights is not None:
        ...
        valid_ops = [
            ...
            if w > 0 and name in AUGMENT_FUNC_MAP
        ]
        if not valid_ops:  # 如果所有权重都是 0 或无效，则回退到所有操作均匀分布
            ...
            population = [
                AUGMENT_FUNC_MAP[name] for name, _ in valid_ops
            ]  # 函数对象列表
...
def augment_cell_multi(
    ...
    for sent in raw_sentences:
        for _ in range(num_variants):
            variant = multi_step_augment(
                sent, min_steps, max_steps, weights=augment_weights
            )
