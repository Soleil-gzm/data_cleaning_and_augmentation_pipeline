...
根据配置的 bucket_config_map（支持正则）为每个桶调用 Data-Juicer 清洗，
生成清洗前后统计、轮次分布对比图、报告等。
"""
...
def clean_bucket(bucket_dir, config_file, output_dir, trace_dir, logger, stats):
    """清洗单个桶目录，更新 stats 字典"""
    ...
    if not input_files:
        logger.warning(f"桶 {bucket_dir.name} 中没有 JSONL 文件")
        return 0
    logger.info(f"  发现 {len(input_files)} 个文件")
    ...
    for input_file in input_files:
        ...
        if result.returncode == 0:
            if output_file.exists():
                ...
                logger.info(f"    ✅ {input_file.name}: {input_cnt} → {output_cnt} 条")
            else:
                output_cnt = 0
                output_turn_dist = defaultdict(int)
                logger.warning(f"    ⚠️ {input_file.name}: 清洗后无输出（所有样本可能被过滤）")
            ...
            logger.error(f"    ❌ {input_file.name} 清洗失败，退出码 {result.returncode}")
            ...
            with open(error_log, 'w') as f:
                ...
                f.write("STDOUT:\n" + result.stdout)
                f.write("\nSTDERR:\n" + result.stderr)
            logger.info(f"        完整错误已保存到 {error_log}")
...
def main():
    ...
    parser.add_argument("--tag", type=str, default="default", help="清洗任务标签（仅独立模式有效）")
    parser.add_argument("--plot_turns", type=str, help="指定需要绘制的轮次列表JSON，如 '[0,1,2,3]'")
    ...
    if args.config_json:
        ...
        step_cfg = config.get('steps', {}).get('03_clean', {})
        
        bucketed_root = step_cfg.get('bucketed_root') or (task_dir / "bucketed")
        cleaned_root = step_cfg.get('cleaned_root') or (task_dir / "cleaned_jsonl")
        trace_root = step_cfg.get('trace_root') or (task_dir / "trace_output")
        configs_dir = step_cfg.get('configs_dir') or "configs/configs_qa"
        bucket_config_map = step_cfg.get('bucket_config_map', [])
        tag = step_cfg.get('tag', task_name)
        ...
        # 设置日志
        logger = setup_logger(task_dir, run_id)
        logger.info(f"任务名称: {task_name}")
        logger.info(f"任务目录: {task_dir}")
    else:
        # 独立命令行模式
        bucketed_root = Path(args.bucketed_root) if args.bucketed_root else None
        cleaned_root = Path(args.cleaned_root) if args.cleaned_root else None
        trace_root = Path(args.trace_root) if args.trace_root else None
        configs_dir = args.configs_dir
        bucket_config_map = json.loads(args.bucket_config_map) if args.bucket_config_map else []
        tag = args.tag
        if not bucketed_root or not cleaned_root:
            print("错误：独立模式需要提供 --bucketed_root 和 --cleaned_root")
            sys.exit(1)
        # 独立模式简单日志
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        run_id = f"{timestamp}_clean_{tag}"
        task_dir = bucketed_root.parent  # 推测任务目录
    ...
    logger.info(f"清洗结果目录: {cleaned_base}")
    logger.info(f"Trace 目录: {trace_base}")
    logger.info(f"报告目录: {report_base}")
    ...
    plot_turns = None
    if args.plot_turns:
        plot_turns = json.loads(args.plot_turns)

    # 遍历所有桶目录
    bucketed_root_path = Path(bucketed_root)
    if not bucketed_root_path.exists():
        logger.error(f"分桶根目录不存在: {bucketed_root_path}")
        sys.exit(1)
    
    for bucket_dir in bucketed_root_path.iterdir():
        ...
        success = clean_bucket(bucket_dir, config_file, output_dir, trace_dir, logger, overall_stats)
    ...
    logger.info("="*60)
    logger.info(f"Run ID: {run_id}")
    logger.info(f"总体输入样本数: {overall_stats['total_input']}")
    logger.info(f"总体输出样本数: {overall_stats['total_output']}")
    if overall_stats['total_input'] > 0:
        logger.info(f"总体保留率: {overall_stats['total_output']/overall_stats['total_input']*100:.2f}%")
    else:
        logger.info("总体保留率: N/A (无输入样本)")
    logger.info("\n各桶统计:")
    for bucket, stats in overall_stats["buckets"].items():
        inp = stats["input_samples"]
        out = stats["output_samples"]
        rate = out/inp*100 if inp>0 else 0
        logger.info(f"  {bucket}: {inp} → {out} ({rate:.2f}%)")
    logger.info(f"\n报告已保存到: {report_base}")
    logger.info("清洗完成！")
